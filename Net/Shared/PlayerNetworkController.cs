...
namespace Aetheris
{
    ...
    public class PlayerNetworkController
    {
        ...
        {
            // CRITICAL: Don't add ourselves as a remote player
            // You'll need to track your own player ID - for now, ignore updates that match our position exactly
            if (Vector3.Distance(player.Position, update.Position) < 0.01f)
            {
                Console.WriteLine($"[Network] Ignoring self-update (ID: {playerId})");
                return;
            }
            ...
            if (!remotePlayers.TryGetValue(playerId, out var remote))
            {
                ...
                Console.WriteLine($"[Network] New player joined: {playerId} at {update.Position}");
            }
